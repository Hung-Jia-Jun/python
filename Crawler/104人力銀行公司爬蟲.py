...
#先爬取士林北投區的公司列表
req = s.get("https://www.104.com.tw/cust/list/ajax/index?page=0&area=6001001009,6001001008&order=1&mode=s&jobsource=checkc")
req.encoding = 'UTF-8'
JsonStr=req.text
...
for Area in AreaLi:
	for page in range(1,int(totalpage)):
		ComDatareq = s.get("https://www.104.com.tw/cust/list/ajax/index?page="+str(page)+"&area="+Area+"&order=1&mode=s&jobsource=checkc", timeout=10)
	...
	for Company_json in Company_jsonLi: 
		...
		for ele in Company_json['data']['list']:
			...
			CustomDetail=s.get(Custom_reqUrl, timeout=10)
