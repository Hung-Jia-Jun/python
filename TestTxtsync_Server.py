...
def Runthread():
	...
	ADDR = ('localhost',47992)  #python 內部的連線port
	tcpSerSock = socket(AF_INET, SOCK_STREAM)
	tcpSerSock.bind(ADDR)
	tcpSerSock.listen(50)
	#pdb.set_trace()
	py_tcpCliSock,addr = tcpSerSock.accept()
	...
	while True:
		...
		while True:
			...
			if Recv_Client_Message[0:9]=="PlaySound": #處理發聲音檔
				...
				localPlaySound_text=PythonConsole+","+LanguageMode+","+PlaySound_text #建構出返回數據  格式為"PythonConsole辨識/發聲,LanguageMode 語言,PlaySound_text  發聲語句,"
				py_tcpCliSock.send(localPlaySound_text)
...
def Python_Client():
	ADDR = ('localhost',47992)  #python 內部通訊
	python_lo_tcpCliSock = socket(AF_INET,SOCK_STREAM)
	python_lo_tcpCliSock.connect(ADDR)



	ADDR = ('localhost',47991)  #python 內部的連線port
	tcpSerSock = socket(AF_INET, SOCK_STREAM)
	tcpSerSock.bind(ADDR)
	tcpSerSock.listen(50)
	user_tcpCliSock,addr = tcpSerSock.accept()
	Temp_Sound=""
	while True:
		python_Recv_Message=python_lo_tcpCliSock.recv(BUFSIZ) #接收指令
		print "local mess:",python_Recv_Message
		python_Recv_Message="Send_To_PythonClient:"+python_Recv_Message
		user_tcpCliSock.send(python_Recv_Message)


threading.Thread(target=Runthread).start()
threading.Thread(target=Python_Client).start()
